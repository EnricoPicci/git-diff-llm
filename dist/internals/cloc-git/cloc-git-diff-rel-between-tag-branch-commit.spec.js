...
describe(`writeAllDiffsForProjectWithExplanationToMarkdown$`, () => {
    it(`should produce a markdown report - the test just tests that function completes without errors`, (done) => {
        ...
        const outDir = './temp';
        (0, cloc_git_diff_rel_between_tag_branch_commit_1.writeAllDiffsForProjectWithExplanationToMarkdown$)(comparisonParams, repoRootFolder, promptTemplates, outDir, languages).subscribe({
            error: (error) => done(error),
            complete: () => done()
        });
    }).timeout(100000);
